...
ConnectionGuard::ConnectionGuard(ConnectionPool &pool, std::unique_ptr<pqxx::connection> connection)
...
ConnectionPool::ConnectionPool(const std::string &connection_string, std::size_t size)
{
    available_.reserve(size);

    for (std::size_t i = 0; i < size; i++)
    {
        available_.push_back(std::make_unique<pqxx::connection>(connection_string));
    }
}
